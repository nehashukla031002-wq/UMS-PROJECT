...
export default function RealChat() {
  ...
  useEffect(() => {
    ...
    // FIX 1: Handle incoming messages - Update unread count and move user to top
    newSocket.on('new-message', (message) => {
      console.log('New message:', message);
      ...
      if (selectedUser && Number(selectedUser.id) === Number(message.fromUserId)) {
        setMessages(prev => [...prev, message]);
        // Reset unread count for this user since chat is open
      ...
      // Move the user who sent message to the top of contacts list
      setUsers(prevUsers => {
        const updatedUsers = [...prevUsers];
        const userIndex = updatedUsers.findIndex(u => u.id === message.fromUserId);
        
        if (userIndex !== -1) {
          const user = updatedUsers[userIndex];
          // Remove user from current position
          updatedUsers.splice(userIndex, 1);
          // Add user at the beginning (top)
          return [user, ...updatedUsers];
        }
        return prevUsers;
      });
      
      toast.success(`New message from ${message.fromUserName || 'someone'}`);
    });
  ...
  useEffect(() => {
    if (socket && selectedUser && isConnected) {
      setMessages([]);
      socket.emit('get-chat-history', selectedUser.id);
      // Reset unread count when opening chat with a user
